...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(self.iterations):
            value = self.values.copy()  # before each iteration, copy one.
            for st in mdp.getStates():
                if mdp.isTerminal(st) == False:
                    maxv = float("-inf")
                    for action in mdp.getPossibleActions(st):
                        v = 0
                        for transition in mdp.getTransitionStatesAndProbs(st, action):
                            v = v + transition[1] * (
                            mdp.getReward(st, action, transition[0]) + discount * self.values[transition[0]])
                        if v > maxv:
                            maxv = v
                    value[st] = maxv
                else:
