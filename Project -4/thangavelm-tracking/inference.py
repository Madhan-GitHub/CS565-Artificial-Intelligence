...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            setGhostPosition = self.setGhostPosition(gameState, pos)
            newPositionDist = self.getPositionDistribution(setGhostPosition)
            oldBelief = self.beliefs[pos]

            for newPos in newPositionDist.keys():
                updatedBeliefs[newPos] += oldBelief * newPositionDist[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while numParticles > 0:
            ...
            if numParticles < len(legalPositions):
                particleList = particleList + \
                    legalPositions[0:numParticles - 1]
                numParticles = 0

            else:
                particleList = particleList + legalPositions
                numParticles = numParticles - len(legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                ...
                counter[particle] = counter[particle] + emissionModel[dist]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            setGhostPosition = self.setGhostPosition(gameState, oldPos)
            newPosDist = self.getPositionDistribution(setGhostPosition)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while self.numParticles >= len(perms):
            self.particles = self.particles + perms
            self.numParticles = self.numParticles - len(perms)

        self.particles = self.particles + perms[0:self.numParticles - 1]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                newPostionDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, oldPos), i, self.ghostAgents[i])
                newParticle[i] = util.sample(newPostionDist)
